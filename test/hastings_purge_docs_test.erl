...
test_purge_local_purge_doc(DbName) ->
    ?_test(begin
        ...
        {ok, LocalPurgeDoc1} = fabric:open_doc(
            DbName,
        ...
        {ok, LocalPurgeDoc2} = fabric:open_doc(
            DbName,
            hastings_util:get_local_purge_doc_id(Idx#h_idx.sig),
            []
        ),
        ...
        {ok, LocalPurgeDoc3} = fabric:open_doc(
            DbName,
            hastings_util:get_local_purge_doc_id(Idx#h_idx.sig),
            []
        ),
...
test_purge_verify_index(DbName) ->
    ?_test(begin
        ...
        {ok, DDoc} = fabric:open_doc(DbName, <<"_design/geodd">>, []),
        {ok, Idx} = hastings_index:design_doc_to_index(DDoc, <<"geoidx">>),
        {ok, LocPurgeDoc} = fabric:open_doc(
            DbName,
            hastings_util:get_local_purge_doc_id(Idx#h_idx.sig),
            []
        ),
...
test_purge_remove_local_purge_doc(DbName) ->
  ?_test(begin
      ...
      {ok, DDoc} = fabric:open_doc(DbName, <<"_design/geodd">>, []),
      {ok, Idx} = hastings_index:design_doc_to_index(DDoc, <<"geoidx">>),
      {ok, LocalPurgeDoc} = fabric:open_doc(
          DbName,
          hastings_util:get_local_purge_doc_id(Idx#h_idx.sig),
          []
      ),
